...
import ScrabbleShitBoard from "@/components/games/scrabble-shit-board"
...
export default function ScrabbleShitPage() {
  ...
  const game = {
    ...
    lore: "Spell words from poop and plunger tiles. FLUSH and CLOG score bonus points. Highest score wins the sewer.",
  }
  ...
  const handleBetPlaced = (amount: number) => {
    ...
    toast({
      title: "Game Started",
      description: `Bet of ${amount} $DATX locked in. Spell to win!`,
    })
  }
      ...
      <div className="relative z-10 flex-1 py-8 px-4 lg:px-6 overflow-y-auto">
        <div className="max-w-7xl mx-auto">
          ...
          <div className="flex flex-col lg:flex-row justify-between items-start gap-8 mb-8">
            <div className="flex-1">
              ...
              <div className="mt-4 text-xs text-muted-foreground space-y-1">
                <p>💰 Bet: 0.06 $DATX per match</p>
                <p>📝 Goal: Highest score wins (500+ points)</p>
                <p>🎯 Drag tiles from rack to board grid</p>
                <p>🌀 FLUSH/CLOG words: +50/+40 bonus</p>
                <p>🫧 Swirl Triple: 3x word score power-up</p>
              </div>
            </div>
          ...
          <Card className="glow-border bg-black/60 backdrop-blur border-2 p-4 lg:p-8 mb-8">
            {hasPlacedBet ? (
              <ScrabbleShitBoard gameBet={betAmount} />
            ) : (
              <div className="flex flex-col items-center justify-center py-12 text-center">
                <p className="text-muted-foreground mb-4">Place a bet to start Scrabble Sludge</p>
                <Button onClick={handleStartBetting} className="bg-primary text-black hover:bg-primary/90 font-bold">
                  Place Bet & Play (0.06 $DATX)
                </Button>
              </div>
            )}
          </Card>
