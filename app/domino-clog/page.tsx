...
import DominoClogBoard from "@/components/games/domino-clog-board"
...
export default function DominoClogPage() {
  ...
  const game = {
    name: "Domino Drain",
    lore: "Poop dominoes clog the chain. Match ends, lowest hand wins.",
  }
  ...
  return (
    <main className="min-h-screen bg-background flex flex-col overflow-hidden">
      ...
      <div
        ...
        style={{
          background: `
            radial-gradient(ellipse at 20% 50%, rgba(0, 255, 100, 0.15) 0%, transparent 40%),
            radial-gradient(ellipse at 80% 80%, rgba(0, 255, 200, 0.1) 0%, transparent 50%),
            radial-gradient(ellipse at 50% 0%, rgba(100, 255, 200, 0.08) 0%, transparent 60%),
            linear-gradient(135deg, #0a1a0f 0%, #0a0a0a 50%, #0a1515 100%)
          `,
        }}
      />
      <div className="fixed inset-0 -z-10 bg-gradient-to-b from-green-900/20 via-transparent to-black/60 backdrop-blur-sm top-20" />
      ...
      <div className="relative z-10 flex-1 py-8 px-4 lg:px-6 overflow-y-auto">
        <div className="max-w-7xl mx-auto">
          ...
          <div className="flex flex-col lg:flex-row justify-between items-start gap-8 mb-8">
            <div className="flex-1">
              ...
              <div className="mt-4 text-xs text-muted-foreground space-y-1">
                <p>💰 Bet: 0.03 $DATX per match</p>
                <p>🎯 Goal: Empty your hand first</p>
                <p>🁢 Double-six dominoes (28 tiles)</p>
                <p>⛔ Match chain ends or draw</p>
                <p>💧 Lowest pip count wins</p>
              </div>
            </div>
          ...
          <Card className="glow-border bg-black/60 backdrop-blur border-2 p-4 lg:p-8 mb-8">
            {hasPlacedBet ? (
              <DominoClogBoard gameBet={betAmount} />
            ) : (
              <div className="flex flex-col items-center justify-center py-12 text-center">
                <p className="text-muted-foreground mb-4">Place a bet to start playing Domino Drain</p>
                <Button onClick={handleStartBetting} className="bg-primary text-black hover:bg-primary/90 font-bold">
                  Place Bet & Play (0.03 $DATX)
                </Button>
              </div>
            )}
          </Card>
