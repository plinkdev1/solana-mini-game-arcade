...
import BackgammonFlushBoard from "@/components/games/backgammon-flush-board"
...
export default function BackgammonFlushPage() {
  ...
  const game = {
    ...
    lore: "Race your pieces home through the sewer before they drown. Roll dice, move wisely, bear off first to flush victory.",
  }
  ...
  const handleBetPlaced = (amount: number) => {
    ...
    toast({
      title: "Game Started",
      description: `Bet of ${amount} $DATX locked in. Race to victory!`,
    })
  }
      ...
      <div
        ...
        style={{
          background: `
            ...
            radial-gradient(ellipse at 50% 0%, rgba(255, 0, 128, 0.08) 0%, transparent 60%),
            linear-gradient(135deg, #1a0f1a 0%, #0a0a0a 50%, #0f1a0a 100%)
          `,
        }}
      />
      <div className="fixed inset-0 -z-10 bg-gradient-to-b from-red-900/40 via-transparent to-black/60 backdrop-blur-sm top-20" />
      ...
      <div className="relative z-10 flex-1 py-8 px-4 lg:px-6 overflow-y-auto">
        <div className="max-w-7xl mx-auto">
          ...
          <div className="flex flex-col lg:flex-row justify-between items-start gap-8 mb-8">
            <div className="flex-1">
              ...
              <div className="mt-4 text-xs text-muted-foreground space-y-1">
                <p>💰 Bet: 0.08 $DATX per match</p>
                <p>🎲 Goal: Bear off all 15 pieces first</p>
                <p>🎯 Roll dice to move pieces around 24-point board</p>
                <p>⚡ Power-up: Plunger Double grants extra roll on doubles</p>
                <p>🏁 First to safely move all pieces home wins!</p>
              </div>
            </div>
          ...
          <Card className="glow-border bg-black/60 backdrop-blur border-2 p-4 lg:p-8 mb-8">
            {hasPlacedBet ? (
              <BackgammonFlushBoard gameBet={betAmount} />
            ) : (
              <div className="flex flex-col items-center justify-center py-12 text-center">
                <p className="text-muted-foreground mb-4">Place a bet to start playing Backgammon Bluff</p>
                <Button onClick={handleStartBetting} className="bg-primary text-black hover:bg-primary/90 font-bold">
                  Place Bet & Play (0.08 $DATX)
                </Button>
              </div>
            )}
          </Card>
