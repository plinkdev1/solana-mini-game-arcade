...
import PegFlushBoard from "@/components/games/peg-flush-board"
...
export default function PegFlushPage() {
  ...
  const game = {
    name: "Peg Plunge",
    lore: "Eliminate pegs until one remains at the center. Outsmart the AI or sink forever.",
  }
  ...
  const handleBetPlaced = (amount: number) => {
    ...
    toast({
      title: "Game Started",
      description: `Bet of ${amount} $DATX locked in. Beat the AI!`,
    })
  }
  ...
  return (
    <main className="min-h-screen bg-background flex flex-col overflow-hidden">
      ...
      <div
        ...
        style={{
          background: `
            radial-gradient(ellipse at 20% 50%, rgba(255, 100, 200, 0.15) 0%, transparent 40%),
            radial-gradient(ellipse at 80% 80%, rgba(150, 100, 255, 0.1) 0%, transparent 50%),
            radial-gradient(ellipse at 50% 0%, rgba(200, 100, 255, 0.08) 0%, transparent 60%),
            linear-gradient(135deg, #1a0f1a 0%, #0a0a0a 50%, #1a0a15 100%)
          `,
        }}
      />
      <div className="fixed inset-0 -z-10 bg-gradient-to-b from-purple-900/20 via-transparent to-black/60 backdrop-blur-sm top-20" />
      ...
      <div className="relative z-10 flex-1 py-8 px-4 lg:px-6 overflow-y-auto">
        <div className="max-w-7xl mx-auto">
          ...
          <div className="flex flex-col lg:flex-row justify-between items-start gap-8 mb-8">
            <div className="flex-1">
              ...
              <div className="mt-4 text-xs text-muted-foreground space-y-1">
                <p>💰 Bet: 0.02 $DATX (vs AI)</p>
                <p>🎯 Goal: Leave 1 peg in center</p>
                <p>🧩 33 pegs start, 1 empty center</p>
                <p>⚡ Pegs "flush" to center hole</p>
                <p>🤖 Greedy AI opponent</p>
              </div>
            </div>
          ...
          <Card className="glow-border bg-black/60 backdrop-blur border-2 p-4 lg:p-8 mb-8">
            {hasPlacedBet ? (
              <PegFlushBoard gameBet={betAmount} />
            ) : (
              <div className="flex flex-col items-center justify-center py-12 text-center">
                <p className="text-muted-foreground mb-4">Place a bet to start playing Peg Plunge against AI</p>
                <Button onClick={handleStartBetting} className="bg-primary text-black hover:bg-primary/90 font-bold">
                  Place Bet & Play (0.02 $DATX)
                </Button>
              </div>
            )}
          </Card>
